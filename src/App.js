...
class App extends Component {
  ...
  onChangeThumbnail = thumbUrl => {
    const {activeImage} = this.state
    const imageValue = imagesList.filter(
      eachImage => eachImage.thumbnailUrl === thumbUrl,
    )
  ...
  render() {
    const {activeTabId, activeImage, time, score, isTrue} = this.state
        ...
        <div className="content-container">
          {!isTrue && (
            <div className="first-content">
              <img src={activeImage} alt="match" className="fruit-image" />
              <ul className="tab-container-list">
                {tabsList.map(eachTab => (
                  <TabItems
                    tabItems={eachTab}
                    key={eachTab.tabId}
                    onChangeTabItems={this.onChangeTab}
